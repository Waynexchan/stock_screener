...
- Execution: next-session open, 5 bps entry and exit slippage, trailing 20-session structural stop, stop-first ambiguity policy, 40-session maximum hold, USD 587 normalized initial risk, maximum four simultaneous positions.
...
`FORWARD_FILTER_AUDIT_V1` froze 208 candidates from the earliest complete snapshots across signal dates 2026-09-04, 2026-09-08, 2026-09-09, and 2026-09-11. As of the data cutoff, zero candidates had a complete five-session outcome. Raw 5/10/20/40-session returns, MFE, MAE, and rolling beta will populate as sessions mature. Executable plan-trigger R outcomes remain explicitly unavailable until their conservative simulator is implemented and tested.
